...
function Index({ rules, count, page, termSearch, can }) {
    ...
    const table = rules.data.map((item, index) => {
        return (
            <tr key={index} className={"border-t transition hover:bg-neutral-100 " + (index % 2 == 0? 'bg-neutral-50': '')}>
                <td className="px-1 py-3 font-light"><Link href={can.view? route('rules.show', item.id): route('rules.index', {term: term, page: currentPage})}>{item.group.description}</Link></td>
                <td className="px-1 py-3 font-light"><Link href={can.view? route('rules.show', item.id): route('rules.index', {term: term, page: currentPage})}>{item.description}</Link></td>
                <td className="px-1 py-3 font-light"><Link href={can.view? route('rules.show', item.id): route('rules.index', {term: term, page: currentPage})}>{item.control}</Link></td>
                <td className="flex justify-end py-3 pr-2 text-neutral-400">
                    <Link href={can.view? route('rules.show', item.id): route('rules.index', {term: term, page: currentPage})}>
    ...
    return (
        <>
            <AuthenticatedLayout titleChildren={'Gerenciamento de Regras'} breadcrumbs={[{ label: 'Regras', url: route('rules.index') }]}>
                <div className="flex gap-2 md:flex-row md:gap-4">
                    {can.create && <Panel className={'inline-flex'}>
                        <Link href={route('rules.create')} className="inline-flex items-center justify-between gap-2 px-3 py-2 font-light text-white transition bg-blue-500 border border-transparent rounded-md focus:ring hover:bg-blue-600 focus:ring-sky-300">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="w-5 h-5" viewBox="0 0 16 16">
                    ...
                    <table className="w-full table-auto text-neutral-600">
                        <thead>
                            <tr className="border-b">
                                <th className="px-1 pt-3 font-semibold text-left">Página</th>
                                <th className="px-1 pt-3 font-semibold text-left">Descrição</th>
                                <th className="px-1 pt-3 font-semibold text-left">Controle</th>
                                <th></th>
                            </tr>
                        </thead>
                        <tbody>
